...
//   const { trips, players: globalPlayers, rounds, foursomes } = useAuth()
...
//   // Filter rounds for this trip
//   const tripRounds = (rounds || []).filter((round: Round) => round.tripId === tripId)
//   const tripFoursomes = (foursomes || []).filter((f: Foursome) => 
//     tripRounds.some((r: Round) => r.id === f.roundId)
//   )
...
//               {tripRounds.map((round: Round) => {
//                 const roundFoursomes = tripFoursomes.filter((f: Foursome) => f.roundId === round.id)
//                 return (
...
//                               {round.format === "2v2" ? "2v2 Best Ball" : "1v1 Singles"} • {round.teeBox || "Default Tee"}
...
//             {/* Foursomes Details */}
//             <Tabs defaultValue={`round-${tripRounds[0]?.id}`} className="space-y-6">
...
//                         </Card>
//                       ) : (
//                         roundFoursomes.map((foursome: Foursome) => {
//                           // Use globalPlayers instead of participants
//                           const playerNames = foursome.players
//                             .map((playerId: string) => globalPlayers.find((p: Player) => p.id === playerId))
//                             .filter(Boolean)
...
//                                   {playerNames.map((player: Player | undefined, idx: number) => (
//                                     <div
//                                       key={idx}
//                                       className="flex items-center justify-between p-3 rounded-lg bg-muted/50 border border-border/50"
//                                     >
//                                       <div className="min-w-0">
//                                         <p className="font-medium text-foreground truncate">{player?.name || "Unknown Player"}</p>
//                                         <p className="text-xs text-muted-foreground">HCP: {player?.handicap || "N/A"}</p>
//                                       </div>
